...
3. **Create context `deploy`** with one env var used by `deploy`:
   - `VPS_KNOWN_HOSTS` — base64 of the VPS host key, so SSH host verification is
     ...
     ```bash
     ssh-keyscan -t ed25519 ${VPS_HOST} | base64 -w0
     ```
...
4. **Add the deploy SSH key.** The keypair is **already generated on the VPS** and
   ...
   Key*, set Hostname `${VPS_HOST}` and paste the **private** key, retrieved on the
   VPS with:
...
When CI runs `ssh rwinter-deploy@${VPS_HOST} "$DIGEST"`, SSH ignores the requested
...
ssh rwinter-deploy@${VPS_HOST} "$REF"
```
